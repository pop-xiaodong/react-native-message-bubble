...
var BubbleText = React.createClass({
  ...
  render() {
    ...
    var {messageType, messages, imageSource} = this.props;
    if (messageType) {
      return (
        <View style={styles.rowRight}>
          ...
          { imageSource && <Image style={[styles.userImage, {marginRight: 10, marginLeft: 3}]} source={imageSource}/> }
        </View>
      )
    } else {
      return (
        <View style={styles.rowLeft}>
          { imageSource && <Image style={[styles.userImage, {marginLeft: 10, marginRight: 3}]} source={imageSource}/> }
          <BubbleView
            text={messages}
            type={false}
            style={{justifyContent: 'center', width: viewWidth+15, height: viewHight-5, opacity: opacity}}
